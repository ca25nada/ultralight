...
function addJudge(pn,judge,isHold)
	--
	if isHold == true then
		if pn == PLAYER_1 then
			if isFailing(PLAYER_1) == false then
				judgeTableP1[#judgeTableP1+1] = judge
				judgeStatsP1[judge] = judgeStatsP1[judge]+1--PHJudge(PLAYER_1,judge)
			end
			curMaxHoldsP1 = curMaxHoldsP1+1
		elseif pn == PLAYER_2 then
			if isFailing(PLAYER_2) == false then
				judgeTableP2[#judgeTableP2+1] = judge
				judgeStatsP2[judge] = PHJudge(PLAYER_2,judge)
			end
		...
		if pn == PLAYER_1 then
			if isFailing(PLAYER_1) == false then
				judgeTableP1[#judgeTableP1+1] = judge
				judgeStatsP1[judge] = judgeStatsP1[judge]+1--PJudge(PLAYER_1,judge)
			end
		...
		elseif pn == PLAYER_2 then
			if isFailing(PLAYER_1) == false then
				judgeTableP2[#judgeTableP2+1] = judge
				judgeStatsP2[judge] = PJudge(PLAYER_2,judge)
			end
			if (judge ~= 'TapNoteScore_HitMine') and (judge ~= 'TapNoteScore_AvoidMine') then
				curMaxNotesP2 = curMaxHoldsP2+1
			else
				curMaxMinesP2 = curMaxMinesP2+1
			end
		end
...
function getJudge(pn,judge)
	if pn == PLAYER_1 then
		return judgeStatsP1[judge] or 99999
	elseif pn == PLAYER_2 then
		return judgeStatsP2[judge] or 99999
	end
	return 0
end
...
function getMaxScore(pn,scoreType) -- dp, ps, migs = 0,1,2 respectively
...
function getCurMaxScore(pn,scoreType)
	local curMaxNotes = getCurMaxNotes(pn)
	local curMaxHolds = getCurMaxHolds(pn)
	if scoreType == 0 then
...
function getCurScore(pn,scoreType)
	if pn == PLAYER_1 then
		if scoreType == 0 then
